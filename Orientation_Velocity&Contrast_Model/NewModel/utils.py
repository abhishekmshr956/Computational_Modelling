...
def plot_data(data_list, label,dt=1/13.0, ori=False,fig_size=(10,5), contrast=False,speed=[15,30,60,120], title='', savefig=False, c=['k','b']):
    ...
    for data in data_list:
        ...
        for i in range(int(n_sti/n_col)):
            for j in range(n_col):
                ax[i, j].plot(x_values, data[:, count], label = label[c_count],color=c[c_count],linewidth=2.0)
                ...
                # else:
                #     if count == n_col:
                #         ax[i,j].legend(loc=1,bbox_to_anchor=(0,0,1,0.9), frameon=False);
            
        c_count += 1
        
    if ori == False:
        f.text(0.01, 0.55, r'Calcium response $(\Delta F/F)$',fontsize=10, va='center', rotation='vertical')
    ...
    if ori == False:
        ...
        for axis,r in zip(ax[:,0], angle):
            axis.set_ylabel(r, rotation=0,fontsize=12, labelpad=10)
    #f.text(0.01,0.98,'Speed(deg/sec.)',fontsize=10)
...
# def plot_data(data_list, dt=1/13.0, fig_size=(10,5), speed=[15,30,60,120], title='', savefig=False, c=['k','b']):
#     """Plots data for multiple datsets"""
#     n_col = len(speed) #number of columns i.e. number of speed stimuli
#     n_sti = data_list[0].shape[1] #number of orientations
#     x_values = np.arange(data_list[0].shape[0]) * dt #changing x-axis to time. multiply with time step dt = 1/frequency
...
def plot_data_twinaxis(data_list, dt=1/13.0, fig_size=(10,5), speed=[15,30,60,120], title='', savefig=False, c=['k','r'],legend=['Arclight','GCaMP'],ylim1=(-0.04,0.08),ylim2=(-20,250),contrast=False):
    """Plots data for multiple datsets"""
    n_col = len(speed) #number of columns i.e. number of speed stimuli
    n_sti = data_list[0].shape[1] #number of orientations
    x_values = np.arange(data_list[0].shape[0]) * dt #changing x-axis to time. multiply with time step dt = 1/frequency
    ...
    for data in data_list:
        ...
        for i in range(int(n_sti/n_col)):
            for j in range(n_col):
                if c_count == 0:
                    ax[i, j].plot(x_values, data[:, count], label=legend[0],color=c[c_count],linewidth=2.0)
                    ...
                    ax1 = ax[i,j].twinx()
                    ax1.plot(x_values, data[:, count], label=legend[1],color=c[c_count],linewidth=2.0)
                    ax1.set_ylim(ylim2)
...
def plot_modelpeaktuning(data_list, speed, color, ylim1=(0.0,0.12),ylim2=(0.0,6.0), fig_size=(3,5.5), title='',savefig=False, contrast=False):
    peak_tuning = np.empty((len(data_list),len(speed)*2)) #first dimesnion is for Arclight, Gcamp, Model respectively. 
                                #second dimension is for n speeds PD first n column, ND last n columns
    n=len(speed)
    for i in range(len(data_list)):
    ...
    f, ax = plt.subplots(2,1,sharex=True,sharey=True,figsize=fig_size)   
    for i in range(peak_tuning.shape[0]):
        ax[0].plot(peak_tuning[0][:n],marker='o',color=color[0],label='GCaMP');
        ax[0].set_ylim(ylim1)
        ax[0].set_ylabel('PD', rotation=0,fontsize=12,labelpad=10)
        #ax1 = ax[0].twinx()
        ax1 = ax[0]
        ax1.plot(peak_tuning[1][:n],marker='o',color=color[1],label='Model');
        #ax1.plot(peak_tuning[2][:n],marker='o',color=color[2]);
        ax1.set_ylim(ylim2)
        #ax1.spines['right'].set_color('blue')
        #ax1.tick_params(axis='y', colors='blue')
        
        ax[1].plot(peak_tuning[0][n:],marker='o',color=color[0],label='GCaMP');
        ax[1].set_ylim(ylim1)
        ax[1].set_ylabel('ND', rotation=0,fontsize=12, labelpad=10)
        #ax2 = ax[1].twinx()
        ax2 = ax[1]
        ax2.plot(peak_tuning[1][n:],marker='o',color=color[1],label='Model');
        #ax2.plot(peak_tuning[2][n:],marker='o',color=color[2]);
        ax2.set_ylim(ylim2);
        if i == 0:
            ax[1].legend(loc=1, frameon=False);
    ...
    if contrast:
        ...
        ax[1].set_xlabel('Speed(deg/sec.)',fontsize=12)
...
def plot_oripeaktuning(data_list, speed, color, label,ylim1=(0,0.08),ylim2=(0,2.5), fig_size=(15,10), title='',savefig=False, contrast = False):
    ...
    def polar_plot(data, rotations_rad, ax, ylim,marker='o', color='r',label=''):
        ...
        ax.plot(rotations_rad, data, marker=marker, color=color, label=label)
        ax.set_ylim(ylim)
    ...
    for i in range(4):
        polar_plot(peak_tuning[0,i,:], rotations_rad, ax[i], ylim=ylim1,color=color[0],label=label[0] );
        polar_plot(peak_tuning[1,i,:], rotations_rad, ax[i], ylim=ylim2,color=color[1],label=label[1] );
...
def plot_Ldir_mag(peak_tuning,speed=[15,30,60,120],title='',savefig=False,contrast=False):
    ...
    #ax.plot(Ldir_mag[1]);
    n = len(speed)
    ax.plot(Ldir_mag[0],marker='o',color='red',label='GCaMP');
    ax.plot(Ldir_mag[1],marker='o',color='green',label='Model');
...
def plot_Ldir(peak_tuning,fig_size=(15,4), title='',savefig=False):
    ...
    ax[0].set_ylabel(r'$\overrightarrow{Ldir}$', rotation=0,fontsize=12, labelpad=60);
    plt.tight_layout();    
    if savefig:
        plt.savefig('figures/'+title+'.pdf',dpi=1000);
